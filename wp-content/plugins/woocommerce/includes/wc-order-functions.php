...
function wc_order_search( $term ) {
	...
	// Search orders.
	if ( is_numeric( $term ) ) {
		$parent_post_ids = array_unique( array_merge(
			$wpdb->get_col(
		...
		/* SELECT wp_posts.ID where post->post_parent IN ($post_ids) */
		$post_ids = array_unique( array_merge(
			$wpdb->get_col(
				$wpdb->prepare( "SELECT DISTINCT ID FROM {$wpdb->posts} WHERE post_parent IN ('" . implode( "','", array_map( 'esc_sql', $parent_post_ids ) ) . "')", wc_clean( $term ) )
			),
			$post_ids,
			array( absint( $term ) )
		) );
