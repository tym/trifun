...
if ( !empty( $results ) ) { ?>
    <table id="yith-wcmbs-reports-table-downloads" class="yith-wcmbs-reports-table-downloads" data-order="<?php echo $order; ?>" data-user-id="<?php echo !empty( $user_id ) ? $user_id : ''; ?>">
        <tr>
            <th><?php _e( 'Product', 'yith-woocommerce-membership' ) ?></th>
            <th><?php
                $arrow = "<span class='dashicons dashicons-arrow-{$arrow_type}'></span>";
                echo '<a id="yith-wcmbs-reports-table-downloads-order-by-downloads" data-order-by="count" data-order="' . $order . '">' . __( 'Downloads', 'yith-woocommerce-membership' ) . $arrow . '</a>';
                ?></th>
        </tr>
