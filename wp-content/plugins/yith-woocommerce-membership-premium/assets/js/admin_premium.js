jQuery( function ( $ ) {
    ...
    $( '.yith-wcmbs-chosen' ).chosen( { width: '100%' } );
    ...
    $( 'select.ajax_chosen_select_customer' ).on( 'yith_wcmbs_chosen_reset', function () {
        $( this ).chosen( 'destroy' );
    ...
    } ).trigger( 'yith_wcmbs_chosen_reset' );

    /**
     * CHOSEN SELECT and DESELECT ALL BUTTON
     */

    $( '.yith-wcmbs-chosen-select-all' ).on( 'click', function ( e ) {
        var target         = $( e.target ),
            container_id   = target.data( 'container-id' ),
            container      = $( '#' + container_id ),
            current_chosen = container.find( '.chosen' ).first();

        current_chosen.find( 'option' ).prop( 'selected', true );
        current_chosen.trigger( 'chosen:updated' );
    } );

    $( '.yith-wcmbs-chosen-deselect-all' ).on( 'click', function ( e ) {
        var target         = $( e.target ),
            container_id   = target.data( 'container-id' ),
            container      = $( '#' + container_id ),
            current_chosen = container.find( '.chosen' ).first();

        current_chosen.find( 'option:selected' ).removeAttr( 'selected' );
        current_chosen.trigger( 'chosen:updated' );
    } );
