...
if ( !class_exists( 'YITH_WCMBS_Admin_Premium' ) ) {
    ...
    class YITH_WCMBS_Admin_Premium extends YITH_WCMBS_Admin {
        ...
        public function __construct() {
            ...
            add_filter( 'yit_fw_metaboxes_type_args', array( $this, 'custom_type_ajax_products' ) );
        ...
        /**
         * Delete transiente after update options of Membership
         */
        ...
        public function premium_settings( $tab ) {
            $new_tab = include( 'plugin-options/settings-premium-options.php' );
         ...
         * @since 1.0.0
         */
        public function custom_type_ajax_products( $args ) {
            if ( isset( $args[ 'type' ] ) && $args[ 'type' ] == 'yith-wcmbs-ajax-products' ) {
                $new_args = array(
            ...
            if ( isset( $args[ 'type' ] ) ) {
                ...
                switch ( $args[ 'type' ] ) {
                    case 'fl-products':
                        ...
                        $args[ 'type' ]                      = 'yith-wcmbs-ajax-products';
                        break;
                    case 'fl-posts':
                        $items = get_posts( array(
                                                'post_type'      => 'post',
                    ...
                    case 'fl-post-cats':
                        ...
                        break;
                    case 'fl-post-tags':
                        $items_array    = get_tags( array( 'fields' => 'id=>name' ) );
                        $args[ 'type' ] = 'chosen';
                        break;
                    case 'fl-product-tags':
                        $items_array    = get_terms( 'product_tag', array( 'fields' => 'id=>name' ) );
                        $args[ 'type' ] = 'chosen';
                        break;
                    case 'fl-membership-plans':
                        $items_array = array();
                ...
                if ( in_array( $args[ 'type' ], array( 'fl-product-cats', 'fl-post-cats' ) ) ) {
                    ...
                    $args[ 'args' ][ 'buttons' ] = array(
                        array(
                            ...
                            'class' => 'button yith-wcmbs-chosen-select-all',
                        ),
                        array(
                            'title' => __( 'Deselect All', 'yith-woocommerce-membership' ),
                            'class' => 'button yith-wcmbs-chosen-deselect-all',
                        ),
                    );
                }
            ...
            if ( $post_type == 'yith-wcmbs-plan' ) {
                if ( isset( $_POST[ 'yit_metaboxes' ] ) ) {
                    ...
                    if ( !empty( $metaboxes[ '_add-products' ] ) ) {
                        $post_ids = array_merge( $post_ids, $metaboxes[ '_add-products' ] );
                    }
                    if ( !empty( $metaboxes[ '_add-posts' ] ) ) {
                        $post_ids = array_merge( $post_ids, $metaboxes[ '_add-posts' ] );
                    }
                    if ( !empty( $metaboxes[ '_add-pages' ] ) ) {
                        $post_ids = array_merge( $post_ids, $metaboxes[ '_add-pages' ] );
                    }
                    ...
                    if ( !empty( $post_ids ) ) {
                        foreach ( $post_ids as $id ) {
                            ...
                            $plans = array_merge( $plans, array( $post_id ) );
        ...
        public function register_metaboxes( $post_type ) {
            if ( in_array( $post_type, YITH_WCMBS_Manager()->post_types ) ) {
                add_meta_box( 'yith-wcmbs-restrict-access-metabox', __( 'Set access', 'yith-woocommerce-membership' ), array(
                    $this,
                    'restrict_access_metabox_render'
                ), null, 'side', 'high' );
            }
            if ( $post_type == 'yith-wcmbs-plan' ) {
                add_meta_box( 'yith-wcmbs-plan-item-order', __( 'Plan Item Order', 'yith-woocommerce-membership' ), array(
                    $this,
                    'plan_item_order_render'
                ), null, 'normal', 'default' );
                add_meta_box( 'yith-wcmbs-shortcode-plan-item-style', __( 'Item list style of the shortcode', 'yith-woocommerce-membership' ), array(
                    $this,
                    'shortcode_plan_item_list_style_render'
                ), null, 'normal', 'default' );
            }
        }
        ...
        public function plan_item_order_render( $post ) {
            ...
            $ordered_items = get_post_meta( $post->ID, '_yith_wcmbs_plan_items', true );
            ...
            wc_get_template( '/metaboxes/plan_item_order.php', $t_args, YITH_WCMBS_TEMPLATE_PATH, YITH_WCMBS_TEMPLATE_PATH );
        }
                    ...
                    'settings'      => array(
                        ...
                        'fields' => array(
                            ...
                            'linked-plans'                => array(
                                ...
                                'type'     => 'fl-membership-plans',
                                'multiple' => true,
                                'std'      => array(),
                            ),
                            'show-contents-in-my-account' => array(
                                'label' => __( 'Show contents in Membership History', 'yith-woocommerce-membership' ),
                                'desc'  => __( 'Select this option if you want to show contents in Membership History.', 'yith-woocommerce-membership' ),
                                'type'  => 'checkbox',
                                'std'   => 'no',
                            ),
                        ),
            ...
            if ( YITH_WCMBS_Products_Manager()->is_allowed_download() ) {
                $extra_fields = array(
                    ...
                    'can-be-accumulated'         => array(
                        ...
                        'desc'  => __( 'Select this option if you want that downloads can be accumulated.', 'yith-woocommerce-membership' ),
                        'type'  => 'checkbox',
                        'std'   => 'no',
                    ),
                );
        ...
        /**
         * Do actions dumplicate_membership
         *
         * @since       1.0.0
         * @author      Leanza Francesco <leanzafrancesco@gmail.com>
         */
                ...
                switch ( $column ) {
                    ...
                    case 'order':
                        ...
                        if ( $order_id > 0 ) {
                            ...
                            if ( $the_order && $the_order->user_id ) {
                            ...
                            if ( !empty( $user_info ) ) {
                                ...
                                if ( $the_order && ( $the_order->billing_first_name || $the_order->billing_last_name ) ) {
                            ...
                            if ( $the_order && $the_order->billing_email ) {
        ...
        public function register_plugin_for_activation() {
            if ( !class_exists( 'YIT_Plugin_Licence' ) ) {
                require_once( YITH_WCMBS_DIR . 'plugin-fw/lib/yit-plugin-licence.php' );
            }
        ...
        public function register_plugin_for_updates() {
            if ( !class_exists( 'YIT_Upgrade' ) ) {
                require_once( YITH_WCMBS_DIR . 'plugin-fw/lib/yit-upgrade.php' );
            }
