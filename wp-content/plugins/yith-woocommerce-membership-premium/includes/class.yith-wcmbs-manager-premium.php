...
class YITH_WCMBS_Manager_Premium extends YITH_WCMBS_Manager {
    ...
    public function __construct() {
        ...
        if ( defined( 'YITH_WCMBS_RESTRICTED_CPT_MANAGEMENT_ENABLED' ) && YITH_WCMBS_RESTRICTED_CPT_MANAGEMENT_ENABLED ) {
            $restricted_custom_post_types = get_option( 'yith-wcpsc-membership-restricted-custom-post-types', array() );
        ...
        if ( $product_id ) {
            if ( !empty( $this->plans ) ) {
                foreach ( $this->plans as $plan ) {
                    $plan_prod_id = $this->get_membership_product_id_for_plan( $plan->ID );
                    if ( $plan_prod_id == $product_id )
                        return $plan->ID;
                }
            }
        }
    ...
    public function get_non_allowed_post_ids_for_user( $user_id ) {
        ...
        if ( $restricted_user_transient && isset( $restricted_user_transient[ $user_id ] ) ) {
            return $restricted_user_transient[ $user_id ];
        }
    ...
    public function get_restricted_items_in_plan( $plan_id, $args = array() ) {
        ...
        foreach ( $plan_ids as $plan_id ) {
            ...
            foreach ( $restricted_post_types as $post_type ) {
                ...
                $plan_items = array_merge( $plan_items, $this_items );
                ...
                if ( $post_type == 'post' && !empty( $plan_post_tags ) ) {
                    $args       = array(
                        'post_type'                  => $post_type,
                        'posts_per_page'             => -1,
                        'post_status'                => 'publish',
            ...
            if ( $include_products ) {
                ...
                $plan_items = array_merge( $plan_items, $products );
