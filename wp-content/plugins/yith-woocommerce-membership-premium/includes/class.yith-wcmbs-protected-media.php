...
class YITH_WCMBS_Protected_Media {
    ...
    public function __construct() {
        ...
        if ( isset( $_GET[ 'protected_media' ] ) ) {
            add_action( 'get_header', array( $this, 'download_protected_media' ), 999 );
        }
    ...
    public function add_restrict_access_in_media_uploader( $form_fields, $post ) {
        ...
        wc_get_template( '/media/restrict_access.php', $t_args, YITH_WCMBS_TEMPLATE_PATH, YITH_WCMBS_TEMPLATE_PATH );
    ...
    public function media_send_to_editor( $html, $send_id, $attachment ) {
        $restrict_access_plan = get_post_meta( $send_id, '_yith_wcmbs_restrict_access_plan', true );
