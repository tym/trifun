...
*/
            ...
            add_action( 'woocommerce_checkout_update_order_meta', array( $this, 'create_progressive_numeration' ), 10, 2 );
            add_action( 'woocommerce_process_shop_order_meta',    array( $this, 'create_progressive_numeration' ), 35, 2 );
            ...
            if( $is_quote_active && $this->is_quote_numeration_enabled() ) {
                ...
                add_action( 'ywraq_after_create_order', array( $this, 'create_quote_progressive_numeration'), 10, 3 );
        ...
        public function create_progressive_numeration( $post_id, $post=array() )
        {
            ...
            if ( is_array( $post ) || is_null( $post ) || ( 'shop_order' == $post->post_type && 'auto-draft' != $post->post_status ) || ( $this->_shop_order_type == $post->post_type ) && ( $post->post_status!='auto-draft' ) ) {
                ...
                $quote_custom_number = get_post_meta( $post_id, $this->_order_quote_number_meta, true );
                $req_status = get_post_meta( $post_id , 'ywraq_raq_status' , true );
               ...
               /* if( !empty( $is_req_a_quote ) && 'yes' == $is_req_a_quote ){
                    $req_status = get_post_meta( $post_id , 'ywraq_raq_status' , true );

                    if( !empty( $req_status ) && 'accepted' == $req_status ){
        ...
        public function create_quote_progressive_numeration( $order_id, $post, $raq ){
            ...
            if( 'ywraq-new' == $order_status && $this->is_quote_numeration_enabled() ){
                ...
                $this->save_custom_quote_meta_order( $order_id );
            }
        ...
        private function save_custom_meta_order( $order_id ){
            ...
            $order_date =   apply_filters('ywson_order_date', wc_get_order($order_id)->order_date, $order_id );
         ...
         *
         */
        private function save_custom_quote_meta_order( $order_id ){

            $order_date =    apply_filters('ywson_quote_order_date', wc_get_order($order_id)->order_date , $order_id );
            ...
            update_post_meta( $order_id, $this->_order_quote_number_meta, $next_num );
            update_post_meta( $order_id, $this->_order_quote_number_complete, $prefix . $next_num . $suffix );
        }
        ...
        private function save_custom_meta_free_order ( $order_id ){
            ...
            $order      =    wc_get_order( $order_id );
            $order_date =  apply_filters('ywson_free_order_date', $order->order_date, $order_id );
        ...
        public function get_custom_order_number ( $order_number, $order =false ) {
            ...
            $is_a_quote =  $req_status = get_post_meta( $order_number , 'ywraq_raq_status' , true );
            if( '' !== $is_a_quote ){

                $order_number_complete= get_post_meta( $order_number, $this->_order_quote_number_complete, true);
                ...
                $order_number_complete = get_post_meta( $order_number, $this->_order_number_complete, true );
        ...
        public function change_order_date( $order_date, $order_id ){
            $is_req_a_quote = get_post_meta( $order_id, 'ywraq_raq', true);
