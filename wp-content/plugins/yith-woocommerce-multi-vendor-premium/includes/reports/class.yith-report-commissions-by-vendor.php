	...
	class YITH_Report_Commissions_By_Vendor extends WC_Admin_Report {
		...
		public function vendors_widget() {
			...
			<div class="section">
				<form method="GET">
					<div>
						<input type="hidden" class="wc-product-search" style="width:203px;" name="vendor_ids" data-placeholder="<?php _e( 'Search for a vendor&hellip;', 'yith-woocommerce-product-vendors' ); ?>" data-action="yith_json_search_vendors" />
		...
		public function prepare_report_data() {
			...
			foreach ( $vendors as $vendor ) {
				...
				foreach ( $commission_ids as $commision_id ) {
					...
					if( ( 'cancelled' == $commission->status || 'refunded' == $commission->status ) ){
						continue;
					}
