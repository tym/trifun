...
class YITH_WCPB_Request_A_Quote_Compatibility {
            ...
            if ( $product && 'yith_bundle' === $product->product_type ) {
                ...
                if ( $bundled_items ) {
                    foreach ( $bundled_items as $item ) {
                        ...
                        $link  = $prod->is_visible() ? get_permalink( $prod->id ) : '#';
                        $title = esc_attr( $prod->get_title() );
                        $image = !$item->hide_thumbnail ? $prod->get_image() : '';
    ...
    public function add_order_item_meta( $values, $cart_item_key, $item_id ) {
        YITH_WCPB_Frontend_Premium()->woocommerce_add_order_item_meta( $item_id, $values, $cart_item_key );
