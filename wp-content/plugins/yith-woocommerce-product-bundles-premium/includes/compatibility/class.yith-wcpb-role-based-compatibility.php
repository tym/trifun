...
class YITH_WCPB_Role_Based_Compatibility {
    ...
    public function __construct() {
        ...
        add_action( 'yith_wcpb_before_get_per_item_price_tot_with_params', array( $this, 'add_regular_price_and_variations_regular_price_actions' ) );
        add_action( 'yith_wcpb_after_get_per_item_price_tot_with_params', array( $this, 'remove_regular_price_and_variations_regular_price_actions' ) );
    ...
    public function return_original_price( $value, $price, $product ) {
        $is_bundle_per_item_pricing = isset( $product->per_items_pricing ) && $product->per_items_pricing;

        return $product->is_type( 'yith_bundle' ) && $is_bundle_per_item_pricing;
    }
    ...
    public function add_regular_price_actions() {
        add_filter( 'woocommerce_get_regular_price', array( YITH_Role_Based_Prices_Product(), 'get_price' ), 5, 2 );
    }

    public function remove_regular_price_actions() {
        remove_filter( 'woocommerce_get_regular_price', array( YITH_Role_Based_Prices_Product(), 'get_price' ), 5, 2 );
    }
    ...
    public function remove_regular_price_and_variations_regular_price_actions() {
        ...
        remove_filter( 'woocommerce_variation_prices_regular_price', array( YITH_Role_Based_Prices_Product(), 'get_price' ), 5, 2 );
