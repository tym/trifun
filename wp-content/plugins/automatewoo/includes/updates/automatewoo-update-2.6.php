...
if ( $workflows ) foreach ( $workflows as $workflow )
{
	 ...
	 */
	if ( in_array( $workflow->get_trigger()->name, [ 'guest_abandoned_cart', 'abandoned_cart' ] )  )
	{
	...
	if ( ! empty($rule_group) )
	{
		...
		update_post_meta( $workflow->id, 'trigger_options', $trigger_options );
		update_post_meta( $workflow->id, 'rule_options', $rules );
	}
