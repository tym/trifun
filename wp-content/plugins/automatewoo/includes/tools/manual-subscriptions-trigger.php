<?php
/**
 * @class 		AW_Tool_Manual_Subscriptions_Trigger
 * @since		2.4.5
 */

class AW_Tool_Manual_Subscriptions_Trigger extends AW_Tool {
	...
	function get_form_fields() {
		...
		$fields[] = ( new AW_Field_Workflow() )
			...
			->add_query_arg( 'meta_query', [[
				'key' => 'trigger_name',
				'value' => [ 'subscription_payment_complete', 'subscription_status_changed' ]
				]]);
	...
	function display_confirmation_screen( $args ) {
		...
		foreach ( $subscriptions as $i => $subscription_id ) {
			...
			echo '#<a href="'. get_edit_post_link( $subscription->id ).'">'.$subscription->id.'</a> for ' . $subscription->get_formatted_billing_full_name();
			echo '<br>';
		}
	...
	function sanitize_args( $args ) {
		...
		if ( isset( $args['subscription_ids'] ) ) {
			$args['subscription_ids'] = aw_clean( $args['subscription_ids'] );
		}
...
return new AW_Tool_Manual_Subscriptions_Trigger();