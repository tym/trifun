<?php
/**
 * @class 		AW_Admin_Settings_Tab_Abstract
 * @package		AutomateWoo/Admin
 */

abstract class AW_Admin_Settings_Tab_Abstract {
	 ...
	 */
	function get_url() {
		return admin_url( 'admin.php?page=automatewoo-settings&tab=' . $this->id );
	}
	...
	function output_settings_form() {
		AW()->admin->get_view( 'settings-form', [
			'tab' => $this
		]);
	}
			...
			<tr id="<?php echo esc_attr( $field['id'] ); ?>_field_row" valign="top">
				<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $field['id'] ); ?>"><?php echo esc_html( $field['title'] ); ?>
						<?php if ( $field['required'] ): ?><span class="aw-required-asterisk"></span></label><?php endif; ?>
	...
	private function _output_tooltip( $field ) {
		...
		echo AW()->admin->help_tip( $field['tooltip'] );
	}
	...
	function save_field( $setting ) {
		...
		switch ( $setting['type'] ) {
			...
			case 'multi_select_countries':
				$value = array_filter( array_map( 'aw_clean', (array) $raw_value ) );
				break;

			default :
				$value = aw_clean( $raw_value );
				break;
		}
	 ...
	 * @param string $more
	 */
	protected function add_message( $strong, $more = '' ) {
		$this->messages[] = [
			'strong' => $strong,
			'more' => $more
		];
	 ...
	 * @return array
	 */
	protected function section_start( $id, $title = '', $description = '' ) {
		$this->settings[] = [
			'type' => 'title',
			'id' => $this->prefix . $id,
			'title' => $title,
	 ...
	 * @return array
	 */
	protected function section_end( $id ) {
		$this->settings[] = [
			'type' => 'sectionend',
			'id' => $this->prefix . $id,
		];
